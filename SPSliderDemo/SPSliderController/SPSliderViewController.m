...
static NSString * const slideSegmentBarItemIdentifier = @"SPSegmentBarItemIdentifier";

#define INDICATOR_HEIGHT 1.5

@interface SPSliderViewController ()<UICollectionViewDataSource, UICollectionViewDelegate,UICollectionViewDelegateFlowLayout, UIScrollViewDelegate>
...
@implementation SPSliderViewController
...
- (instancetype)initWithViewControllers:(NSArray *)viewControllers
{
    ...
    if (self) {
        ...
        _segmentBarType = SPSegmentBarTypeDynamicWidth;
...
- (void)setupSubviews
{
    if (self.segmentBarType != SPSegmentBarTypeDynamicWidth) {
...
{
    SPSlideSegmentBarItem *segmentBarItem = [collectionView dequeueReusableCellWithReuseIdentifier:slideSegmentBarItemIdentifier forIndexPath:indexPath];
...
- (void)adjustIndicatorBgViewWithDestinationIndex:(NSInteger)destinationIndex percent:(CGFloat)percent
{
    ...
    UIColor *destinationColor = [UIColor colorWithRed:(26.0f + (233.0f - 26.0f) * percent) / 255.0f
                                                green:(26.0f + (71.0f - 26.0f) * percent) / 255.0
                                                 blue:(26.0f + (9.0f - 26.0f) * percent) / 255.0f
                                                alpha:1.0f];
    UIColor *presentColor = [UIColor colorWithRed:(233.0f - (233.0f - 26.0f) * percent) / 255.0f
                                            green:(15.0f - (71.0f - 26.0f) * percent) / 255.0
                                             blue:(9.0f - (9.0f - 26.0f) * percent) / 255.0f
                                            alpha:1.0f];
    
    SPSlideSegmentBarItem *destinationItemCell = (SPSlideSegmentBarItem *)[self.segmentBar cellForItemAtIndexPath:[NSIndexPath indexPathForRow:destinationIndex inSection:0]];
    destinationItemCell.titleLabel.textColor = destinationColor;
    SPSlideSegmentBarItem *presentItemCell = (SPSlideSegmentBarItem *)[self.segmentBar cellForItemAtIndexPath:[NSIndexPath indexPathForRow:self.presentViewIndex inSection:0]];
...
- (UIView *)scrollViewFirstPage
{
    if (_scrollViewFirstPage == nil) {
        ...
        //        _scrollViewFirstPage.backgroundColor = [UIColor purpleColor];
    }
...
- (UIView *)scrollViewMiddlePage
{
    if (_scrollViewMiddlePage == nil) {
        ...
        //        _scrollViewMiddlePage.backgroundColor = [UIColor lightGrayColor];
    }
...
- (UIView *)scrollViewLastPage
{
    if (_scrollViewLastPage == nil) {
        ...
        //        _scrollViewLastPage.backgroundColor = [UIColor yellowColor];
    }
...
- (UICollectionView *)segmentBar
{
    if (!_segmentBar) {
        ...
        [_segmentBar registerClass:[SPSlideSegmentBarItem class] forCellWithReuseIdentifier:slideSegmentBarItemIdentifier];
...
- (UIView *)indicatorBgView
{
    if (!_indicatorBgView) {
        ...
        _indicatorBgView.backgroundColor = [UIColor redColor];
    }
...
- (NSMutableArray *)indicatorBgFrames
{
    if (_indicatorBgFrames == nil) {
        ...
        for (NSInteger i = 0; i < self.viewControllers.count; i++) {
            ...
            if (self.segmentBarType == SPSegmentBarTypeDynamicWidth) {
                width = [title spSizeWithFont:[UIFont systemFontOfSize:16] maxSize:CGSizeMake(MAXFLOAT, [[UIFont systemFontOfSize:14] lineHeight]) lineBreakMode:NSLineBreakByWordWrapping].width + self.indicatorInsets.left + self.indicatorInsets.right;
            }
